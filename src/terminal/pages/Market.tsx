  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-12 gap-3 overflow-auto pb-10">
      <div className="col-span-12 lg:col-span-12">
        <Panel title="GLOBAL SECTOR PERFORMANCE (US SPDR ETFS)">
            ...
            {sectors.map((s) => (
              ...
              >
                <div className="flex flex-col">
                    ...
                    <span className="text-[10px] opacity-70 truncate">{s.shortName?.replace('Select Sector SPDR Fund', '') || 'Sector'}</span>
                </div>
      ...
      <div className="col-span-12 min-h-[260px]">
        <Panel title="MAJOR INDICES & GLOBAL BENCHMARKS">
          <table className="w-full text-[12px]">
            <thead className="text-[10px] tracking-widest text-(--color-term-muted) bg-(--color-term-bg) z-10 sticky top-0">
              <tr className="border-b border-(--color-term-border)">
                <th className="px-4 py-3 text-left">INDEX SYMBOL</th>
            ...
            <tbody>
              {indices.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5 text-right tabular-nums font-semibold">{r.regularMarketPrice?.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>

  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setUser(uRes);
        if (sRes?.value) setSettings(prev => ({ ...prev, OPENROUTER_API_KEY: sRes.value }));
      } catch (err) {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">系統設定 (Settings)</h1>
        <button 
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Profile */}
        <Panel title="個人帳戶" icon={<User className="h-4 w-4" />}>
           <div className="p-4 space-y-4">
              <div>
                 <label className="text-[10px] text-(--color-term-muted) uppercase tracking-widest block mb-1">姓名</label>
                 <div className="text-sm font-medium">{user?.name || '未設定'}</div>
              </div>
              <div>
                 <label className="text-[10px] text-(--color-term-muted) uppercase tracking-widest block mb-1">電子郵件</label>
                 <div className="text-sm font-medium">{user?.email}</div>
              </div>
              <div className="pt-2">
                 <button 
                   onClick={async () => {
                     const newName = prompt('請輸入新姓名', user?.name || '');
                 ...
                 >
                   <Edit2 className="h-3 w-3" /> 修改個人資料
                 </button>
              </div>
           </div>
        </Panel>
        ...
        {/* Subscription */}
        <Panel title="訂閱狀態" icon={<CreditCard className="h-4 w-4" />}>
           <div className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                 <span className="text-sm">當前方案</span>
                 <span className="bg-sky-500/20 text-sky-400 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-widest font-bold">
                    {user?.tier || 'Free'}
                 </span>
              </div>
              <p className="text-xs text-(--color-term-muted)">體驗版用戶，每日 AI 分析額度剩餘 50 次。</p>
              <button 
              ...
              >
                 聯繫商務團隊 (Contact Sales)
              </button>
           </div>
        </Panel>

        {/* AI & Integration */}
        <Panel title="AI 模型與整合" icon={<Key className="h-4 w-4" />} className="md:col-span-2">
           <div className="p-4 space-y-6">
              <div>
                ...
                <p className="text-[10px] text-(--color-term-muted) mt-2 italic">
                  * 啟用「個股 AI 摘要」與「Hermes 智能助手」對話功能所需的金鑰。
                </p>
              </div>

              <div className="flex items-center justify-between border-t border-(--color-term-border) pt-4">
                <div>
                   <div className="text-sm font-medium">通知推送</div>
                   <div className="text-xs text-(--color-term-muted)">當個股觸發預設警報時發送瀏覽器通知</div>
                </div>
                <div 
                  className={cn("w-10 h-5 rounded-full relative cursor-pointer transition-colors", settings.NOTIFICATION_ENABLED ? 'bg-sky-500' : 'bg-zinc-700')}
                  onClick={() => setSettings({...settings, NOTIFICATION_ENABLED: !settings.NOTIFICATION_ENABLED})}
                >
                   <div className={cn("absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform", settings.NOTIFICATION_ENABLED ? 'translate-x-5' : 'translate-x-0')} />
                </div>
              </div>
           </div>
        </Panel>

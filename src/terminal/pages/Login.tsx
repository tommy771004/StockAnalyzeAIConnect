      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-[11px] tracking-widest text-(--color-term-muted)">
            AUTONOMOUS_TRADING_SYNDICATE_v3
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <AnimatePresence>
            {isRegister && (
              ...
              >
                <label className="text-[10px] tracking-widest text-(--color-term-muted)">OPERATOR_ID</label>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] tracking-widest text-(--color-term-muted)">ACCESS_COORD (EMAIL)</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="sys.operator@fin.local"
                className="h-10 w-full border border-(--color-term-border) bg-(--color-term-surface) pl-10 pr-4 text-[13px] tracking-widest text-(--color-term-text) placeholder:text-(--color-term-subtle) focus:border-(--color-term-accent) focus:outline-none transition-colors"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] tracking-widest text-(--color-term-muted)">SECURITY_KEY</label>
            <div className="relative">
          ...
          >
            <span className={cn("relative z-10", loading && "invisible")}>
              {isRegister ? 'ESTABLISH_UPLINK' : 'INITIATE_HANDSHAKE'}
            </span>
            {loading && (
          ...
          <div className="text-center">
            ...
            >
              [ {isRegister ? 'SWITCH_TO_LOGIN' : 'REQUEST_NEW_ACCESS'} ]
            </button>
          </div>
        </form>
      </motion.div>

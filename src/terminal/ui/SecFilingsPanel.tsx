...
export function SecFilingsPanel({ symbol }: Props) {
  ...
  return (
    <Panel
      title={t('edgar.title', 'SEC EDGAR — {{symbol}}', { symbol })}
    ...
    >
      {loading && (
        <div className="flex flex-1 items-center justify-center gap-2 py-8 text-(--color-term-muted) text-[12px]">
          ...
          <span>{t('edgar.loading', '正在從 SEC 載入申報資料...')}</span>
        </div>
      )}
      ...
      {data && (
        <div className="flex flex-col gap-0">
          ...
          <div className="border-b border-(--color-term-border) px-4 py-3 flex items-start justify-between gap-2">
            ...
            >
              {t('edgar.secLink', 'SEC')} <ExternalLink size={10} />
            </a>
          </div>
          ...
          {data.financials && (
            <div className="border-b border-(--color-term-border) px-4 py-3">
              <div className="text-[10px] tracking-widest text-(--color-term-muted) mb-2">
                {t('edgar.financialSummary', '年報財務摘要 (FY {{period}})', { period: data.financials.period })}
              </div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                {([
                  [t('edgar.revenue', '營收'), data.financials.revenue],
                  [t('edgar.netIncome', '淨利'), data.financials.netIncome],
                  [t('edgar.eps', 'EPS'), data.financials.eps],
                  [t('edgar.assets', '資產'), data.financials.assets],
                  [t('edgar.equity', '淨值'), data.financials.equity],
                ] as [string, string][]).map(([label, value]) => (
                  <div key={label} className="flex items-center justify-between">
                    <span className="text-[11px] text-(--color-term-muted)">{label}</span>
                    <span className={cn(
                      'text-[12px] font-semibold tabular-nums',
                      label === t('edgar.netIncome', '淨利') && data.financials!.rawNetIncome != null
              ...
              return (
                <li key={i} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/5 transition-colors">
                  ...
                  <a
                    ...
                    title={t('edgar.openFiling', '開啟申報文件')}
                  >
                    <FileText size={14} />
                  </a>
                </li>

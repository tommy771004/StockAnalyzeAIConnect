...
export default function BacktestPage({ initialSymbol }: { initialSymbol?: string } = {}) {
  ...
  const handleCompare = async () => {
    ...
    if (!sym) { setError(t('backtestEngine.err.enterSymbol', '請輸入股票代碼')); return; }
    if (sym !== symbol) setSymbol(sym);
    if (new Date(period1) >= new Date(period2)) { setError(t('backtestEngine.err.dateOrder', '開始日期必須早於結束日期')); return; }
  ...
  const handleRun = async () => {
    const sym = await resolveInputSymbol();
    if (!sym) { setError(t('backtestEngine.err.enterSymbolCrypto', '請輸入股票/加密貨幣代碼')); return; }
    if (sym !== symbol) setSymbol(sym);
    if (new Date(period1) >= new Date(period2)) { setError(t('backtestEngine.err.dateOrder', '開始日期必須早於結束日期')); return; }
    ...
    try {
      ...
      if (!r || typeof r !== 'object') throw new Error(t('backtestEngine.err.badFormat', '伺服器回傳格式錯誤'));
      const safe = normalizeBacktestResult(r, strategy);
      if (safe.equityCurve.length === 0) throw new Error(t('backtestEngine.err.noData', '該時間區間內無足夠歷史資料，請擴大日期範圍（建議至少6個月）'));
      setResult(safe);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : t('backtestEngine.err.runFailed', '回測執行失敗，請稍後再試'));
    } finally {
      setRunState('idle');
    }
  };
  ...
  const exportCSV = () => {
    ...
    const rows = result.trades.map((tr) => `${tr.entryTime},${tr.exitTime},${tr.entryPrice},${tr.exitPrice},${tr.amount},${tr.holdDays},${tr.pnlPct}%,${tr.pnl},${tr.result === 'WIN' ? '獲利' : '虧損'}`);
    ...
    >
      ...
      {error && (
        <div className="rounded-xl p-4 shrink-0 flex items-start gap-3" style={{ background: 'rgba(255,77,79,0.08)', border: '1px solid rgba(255,77,79,0.25)', color: 'var(--md-error)' }}>
          ...
          <div>
            <div className="font-bold mb-0.5">{t('backtestEngine.failedTitle', '回測失敗')}</div>
            <div>{error}</div>
          </div>
        </div>
      )}
        ...
        <div className="shrink-0 glass-card rounded-[2.5rem] p-6 lg:p-10 shadow-2xl animate-in zoom-in-95 duration-500 relative z-10 overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-2xl font-black tracking-tight" style={{ color: 'var(--md-on-surface)', fontFamily: 'var(--font-heading)' }}>{t('backtestEngine.compareTitle', '多策略績效矩陣')}</h3>
            ...
            <button type="button" onClick={() => { setCompareMode(false); setCompareResults({}); }}
              ...
              {t('backtestEngine.closeCompare', '關閉比較')}
            </button>
          </div>
            ...
            <table className="w-full text-sm mb-6 min-w-[1040px]">
              <thead>
                <tr className="border-b label-meta font-black uppercase tracking-[0.2em]" style={{ borderColor: 'var(--md-outline-variant)', color: 'var(--md-outline)' }}>
                  <th className="pb-6 text-left pl-4">{t('backtestEngine.col.strategy', '策略名稱')}</th>
                  <th className="pb-6 text-right">{t('backtestEngine.col.roi', '總報酬率 (ROI)')}</th>
                  <th className="pb-6 text-right">{t('backtestEngine.col.sharpe', '夏普比率 (Sharpe)')}</th>
                  <th className="pb-6 text-right">{t('backtestEngine.col.mdd', '最大回撤 (MDD)')}</th>
                  <th className="pb-6 text-right">{t('backtestEngine.col.winRate', '勝率 (Win Rate)')}</th>
      ...
      {result ? (
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 px-1">
            {result.regime && (() => {
              const regimeCfg = {
                bull:     { icon: <TrendingUp size={13} />, label: t('backtestEngine.regime.bull', '多頭行情'), bg: 'rgba(82,196,26,0.08)', border: 'rgba(82,196,26,0.25)', color: 'var(--color-down)' },
                bear:     { icon: <TrendingDown size={13} />, label: t('backtestEngine.regime.bear', '空頭行情'), bg: 'rgba(255,77,79,0.08)', border: 'rgba(255,77,79,0.25)', color: 'var(--color-up)' },
                sideways: { icon: <Minus size={13} />, label: t('backtestEngine.regime.sideways', '震盪整理'), bg: 'rgba(245,158,11,0.08)', border: 'rgba(245,158,11,0.25)', color: '#f59e0b' },
              }[result.regime];
              return (
                <div className="flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-widest"
                  ...
                  {regimeCfg.icon}
                  <span>{t('backtestEngine.marketState', '市場狀態')}：{regimeCfg.label}</span>
                </div>
              );
            })()}
            <button type="button" onClick={() => void handleOptimize()} disabled={optimizing}
              ...
              <Zap size={13} />
              {optimizing ? t('backtestEngine.optimizing', '優化中…') : t('backtestEngine.optimize', '自動優化參數')}
            </button>
          </div>
            ...
            <div className="glass-card rounded-[2.5rem] p-6 shadow-xl animate-in fade-in slide-in-from-top-2 duration-500 relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-black tracking-tight" style={{ color: 'var(--md-on-surface)' }}>{t('backtestEngine.betterParams', '找到更優參數')}</div>
                    <div className="text-[10px] font-bold mt-0.5" style={{ color: 'var(--color-down)' }}>
                      {t('backtestEngine.improvement', 'ROI 預計改善 +{{pct}}%，風險評分 {{score}}', { pct: optimizeProposal.improvementPct, score: optimizeProposal.riskAdjustedScore })}
                    </div>
                  </div>
                </div>
                <button type="button"
                  ...
                  <CheckCircle size={13} />
                  {t('backtestEngine.applyParams', '套用參數')}
                </button>
              </div>
          ...
          {optimizeProposal === 'none' && (
            <div className="rounded-2xl px-5 py-3 text-xs font-bold animate-in fade-in duration-300"
              ...
              {t('backtestEngine.noImprovement', '已掃描 5 組變異參數，當前配置已是最佳，無明顯改善空間。')}
            </div>
          )}
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-12 py-20 animate-in fade-in zoom-in-95 duration-1000">
          <div className="text-center space-y-6 max-w-2xl px-6">
            ...
            <div className="space-y-2">
              <h3 className="text-3xl font-black tracking-tight" style={{ color: 'var(--md-on-surface)', fontFamily: 'var(--font-heading)' }}>{t('backtestEngine.emptyTitle', '準備好驗證你的交易策略了嗎？')}</h3>
              <p className="font-medium leading-relaxed" style={{ color: 'var(--md-outline)' }}>
                {t('backtestEngine.emptyDesc', '回測引擎允許你使用歷史市場數據來模擬交易表現。雖然過去的績效不保證未來結果，但它是優化策略、建立信心的關鍵步驟。')}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-5xl px-6">
            {STRATEGIES.map((s, idx) => (
              <button key={`strat-btn-${idx}`} type="button" onClick={() => setStrategy(s.id)}
                ...
                <div className="text-base font-black mb-2" style={{ color: 'var(--md-on-surface)' }}>{t(`backtestEngine.strategy.${s.id}.label`, s.label)}</div>
                <div className="text-xs leading-relaxed font-medium line-clamp-3" style={{ color: 'var(--md-outline)' }}>{t(`backtestEngine.strategy.${s.id}.desc`, s.desc)}</div>
                <div className="label-meta mt-4 font-black uppercase tracking-widest" style={{ color: s.color }}>{t(`backtestEngine.strategy.${s.id}.type`, s.type)}</div>
              </button>
            ))}
          </div>
          ...
          <div className="rounded-3xl p-6 max-w-2xl w-full mx-6" style={{ background: 'rgba(255,183,131,0.05)', border: '1px solid rgba(255,183,131,0.15)' }}>
            <div className="flex items-center gap-3 text-sm font-black uppercase tracking-widest mb-3" style={{ color: 'var(--md-tertiary)' }}>
              <AlertCircle size={18} /> {t('backtestEngine.disclaimerTitle', '投資風險免責聲明')}
            </div>
            <p className="text-xs leading-relaxed font-medium" style={{ color: 'var(--md-outline)' }}>
              {t('backtestEngine.disclaimerBody', '本工具提供的回測結果僅供學術研究與策略開發參考。市場環境瞬息萬變，歷史數據無法完全預測未來走勢。所有交易決策應由投資者自行評估，本平台不承擔任何因使用本工具而產生的投資損失。')}
            </p>
          </div>
        </div>
      )}

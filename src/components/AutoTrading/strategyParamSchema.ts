...
export const STRATEGY_PARAM_SCHEMA: Record<StrategyType, StrategyParamFieldSchema[]> = {
  RSI_REVERSION: [
    { path: 'RSI_REVERSION.period', label: 'Period', type: 'number', step: 1, min: 2, max: 200, defaultValue: 14 },
    { path: 'RSI_REVERSION.oversold', label: 'Oversold', type: 'number', step: 1, min: 1, max: 50, defaultValue: 30 },
    { path: 'RSI_REVERSION.overbought', label: 'Overbought', type: 'number', step: 1, min: 50, max: 99, defaultValue: 70 },
  ],
  BOLLINGER_BREAKOUT: [
    { path: 'BOLLINGER_BREAKOUT.period', label: 'Period', type: 'number', step: 1, min: 2, max: 200, defaultValue: 20 },
    { path: 'BOLLINGER_BREAKOUT.stdDev', label: 'StdDev', type: 'number', step: 0.1, min: 0.5, max: 5, defaultValue: 2 },
  ],
  MACD_CROSS: [
    { path: 'MACD_CROSS.fast', label: 'Fast', type: 'number', step: 1, min: 2, max: 80, defaultValue: 12 },
    { path: 'MACD_CROSS.slow', label: 'Slow', type: 'number', step: 1, min: 4, max: 200, defaultValue: 26 },
    { path: 'MACD_CROSS.signal', label: 'Signal', type: 'number', step: 1, min: 2, max: 60, defaultValue: 9 },
  ],
  AI_LLM: [
    { path: 'AI_LLM.confidenceThreshold', label: 'Min Confidence Threshold', type: 'range', step: 1, min: 50, max: 95, defaultValue: 65, fullWidth: true, unit: '%' },
  ],
};

export const RISK_EXIT_PARAM_SCHEMA: StrategyParamFieldSchema[] = [
  { path: 'stopLossPct', label: 'Stop Loss', type: 'number', step: 0.1, min: 0.1, max: 50, defaultValue: 5, unit: '%' },
  { path: 'takeProfitPct', label: 'Take Profit', type: 'number', step: 0.1, min: 0.1, max: 100, defaultValue: 10, unit: '%' },
  { path: 'trailingStopPct', label: 'Trailing Stop', type: 'number', step: 0.1, min: 0.1, max: 50, defaultValue: 3, unit: '%' },
];

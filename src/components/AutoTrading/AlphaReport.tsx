...
export function AlphaReport() {
  ...
  if (loading) {
    return (
      <div className="h-[400px] flex flex-col items-center justify-center space-y-6">
        ...
        <div className="text-[11px] font-bold text-white uppercase tracking-[0.2em] animate-pulse">Evolving Report Data...</div>
      </div>
    );
  }
  ...
  const AchievementIcon = ({ type }: { type: string }) => {
    switch(type) {
      case 'CRITICAL_DEFENSE': return <div className="flex items-center gap-1.5 bg-rose-500/20 text-rose-400 px-2 py-1 rounded-sm border border-rose-500/30 text-[9px] font-bold"><ShieldCheck className="h-3 w-3" /> CRITICAL_DEFENSE</div>;
      case 'ALPHA_CATCHER': return <div className="flex items-center gap-1.5 bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded-sm border border-emerald-500/30 text-[9px] font-bold"><TrendingUp className="h-3 w-3" /> ALPHA_CATCHER</div>;
      case 'STREAK_5': return <div className="flex items-center gap-1.5 bg-amber-500/20 text-amber-400 px-2 py-1 rounded-sm border border-amber-500/30 text-[9px] font-bold"><Trophy className="h-3 w-3" /> WIN_STREAK</div>;
      default: return null;
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide">
         <span className="text-[10px] text-white/30 uppercase font-bold whitespace-nowrap">Achievements:</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-black/60 border border-violet-500/20 p-6 rounded-sm relative overflow-hidden group">
          ...
          <div className="text-[9px] text-violet-400 uppercase font-bold mb-2 tracking-widest">AI Value Added</div>
          <div className="text-3xl font-mono text-white">+{report.aiValueAdded.toLocaleString()} <span className="text-[10px] opacity-40">TWD</span></div>
        ...
        <div className="md:col-span-2 bg-black/40 border border-white/5 p-6 rounded-sm space-y-4">
           <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-white uppercase tracking-wider">Neural Confidence Evolution</span>
        ...
        <div className="md:col-span-2 bg-black/40 border border-white/5 rounded-sm overflow-hidden">
          <div className="bg-white/[0.03] px-5 py-3 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold text-white uppercase tracking-widest">Your AI Companion's Journal</span>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-black/40 border border-white/5 p-5 rounded-sm space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-bold text-white uppercase tracking-wider">Attribution Matrix</span>
            </div>
          ...
          <div className="bg-gradient-to-br from-violet-600/20 to-indigo-600/20 border border-violet-500/30 p-5 rounded-sm text-center">
             ...
             <div className="text-[11px] font-bold text-white uppercase mb-1">Growth Status: Advancing</div>

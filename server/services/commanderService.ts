...
const buildSystemPrompt = (currentConfig: unknown) => `你是一位資深的自動交易系統管理員。
你的任務是解析用戶的自然語言指令，並將其轉換為系統配置修改。

[目前的配置]: ${JSON.stringify(currentConfig)}
...
export async function processCommanderCommand(userId: string, command: string) {
  ...
  try {
    const { text } = await callLLM({
      systemPrompt: buildSystemPrompt(currentConfig),
      prompt: command,
      jsonMode: true,
      userId
    });
    ...
    // 執行修改 (內部會進行 zod 驗證)
    await tradingSessionRegistry.update(userId, patch);
